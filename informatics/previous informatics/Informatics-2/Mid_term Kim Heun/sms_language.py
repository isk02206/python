...
def removeVowels(x):
    ...
    shorten_word=''
    for character in x[1:]:   #remove vowel from second letter
        if not character in vowel:
            shorten_word += character
    return x[0] + shorten_word #do not remove the first letter even if the first one is vowel + the word without vowel
